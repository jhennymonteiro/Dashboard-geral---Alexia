...
export const LeadModal: React.FC<LeadModalProps> = ({
  ...
  useEffect(() => {
    if (leadToEdit) {
      ...
      setQueixaCliente(leadToEdit.servico || leadToEdit.queixaCliente || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const selectedService = finalQueixa || 'Não informado';

    const priorStage = leadToEdit ? (leadToEdit.fase !== 'Negócio Perdido' ? leadToEdit.fase : leadToEdit.faseAnterior) : undefined;
    const finalFaseAnterior = fase === 'Negócio Perdido' ? (priorStage || 'Follow Up') : (leadToEdit?.faseAnterior || fase);

    onSave({
      ...(leadToEdit ? { id: leadToEdit.id, createdAt: leadToEdit.createdAt } : { createdAt: new Date().toISOString().split('T')[0] }),
      nome: nome.trim(),
      whatsapp: whatsapp.trim(),
      fase,
      faseAnterior: finalFaseAnterior,
      valorEstimado: typeof valorEstimado === 'number' ? valorEstimado : 0,
      servico: selectedService,
      queixaCliente: selectedService,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-lg w-full shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden my-8 transition-colors duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="relative">
                <DollarSign className="w-4 h-4 text-slate-400 dark:text-slate-500 absolute left-3 top-1/2 -translate-y-1/2" />
                <input
                  type="number"
                  min="0"
                  step="100"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full text-xs px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-slate-900 dark:text-slate-100 transition-all font-semibold"
              >
                {FUNNEL_STAGES.map((s) => (
            ...
            <div>
              ...
              <select
                ...
                className="w-full text-xs px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-slate-900 dark:text-slate-100 transition-all"
              >
              ...
              {origemLead === 'OUTRO' && (
                <input
                  ...
                  className="w-full mt-2 text-xs px-3 py-1.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100"
                />
              )}
            </div>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="OUTRO">+ Novo Serviço...</option>
            </select>
            {queixaCliente === 'OUTRO' && (
              <input
                type="text"
