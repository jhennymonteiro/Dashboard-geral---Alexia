    ...
    <div className="bg-white dark:bg-slate-800/90 rounded-2xl p-6 border border-slate-200/80 dark:border-slate-700/80 shadow-xs flex flex-col justify-between h-full transition-colors duration-200">
      <div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${
              ...
              {hasLoss ? <AlertTriangle className="w-5 h-5" /> : <CheckCircle2 className="w-5 h-5" />}
            </div>
          ...
          <span className={`text-xs font-bold px-3 py-1 rounded-full ${
            ...
            {hasLoss ? 'Maior Evasão' : 'Sem Evasão'}
          </span>
        </div>
        ...
        {hasLoss ? (
          ...
          <div className="border border-emerald-200/90 dark:border-emerald-900/60 bg-emerald-50/40 dark:bg-emerald-950/20 rounded-xl p-4 sm:p-5 space-y-4">
            <div className="flex items-center justify-between">
              <span className="font-bold text-emerald-900 dark:text-emerald-200 text-sm sm:text-base">
                Nenhum gargalo de evasão
              </span>

              <span className="bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-800/80 text-emerald-600 dark:text-emerald-400 text-xs sm:text-sm font-bold px-2.5 py-1 rounded-md shadow-2xs">
                0.0%
              </span>
            </div>

            <div className="grid grid-cols-2 gap-4 pt-2 border-t border-emerald-200/60 dark:border-emerald-900/40">
              <div>
                <div className="text-xs font-medium text-slate-500 dark:text-slate-400">
                  Quantidade Perdida
                </div>
                <div className="text-2xl font-bold text-slate-900 dark:text-slate-100 tracking-tight mt-1">
                  0 <span className="text-xs font-normal text-slate-500">leads</span>
                </div>
              </div>
        ...
        <div className="bg-slate-950 dark:bg-slate-900 text-white rounded-xl p-4 flex items-start gap-3 shadow-xs">
          {hasLoss ? (
            ...
            <>
              ...
              <p className="text-xs text-slate-200 dark:text-slate-300 leading-relaxed font-medium">
                Nenhum lead descartado ou em Negócio Perdido até o momento. Todos os contatos estão ativos ou foram concluídos com sucesso.
              </p>
            </>
          )}
        </div>
