    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs p-5 lg:p-6 flex flex-col justify-between h-full transition-colors duration-200">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Distribuição por canal de captação</p>
        </div>
              ...
              >
                {sources.map((entry) => (
                  <Cell
                    key={entry.name}
                    fill={entry.color}
                    stroke={selectedOrigem === entry.name ? '#38bdf8' : 'transparent'}
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between p-2 rounded-lg text-xs transition-all cursor-pointer ${
                  isSelected
                    ? 'bg-blue-50 dark:bg-blue-950/80 border border-blue-200 dark:border-blue-800 text-blue-900 dark:text-blue-200 font-bold'
              ...
              >
                ...
                <div className="font-mono text-slate-900 dark:text-slate-100 font-semibold shrink-0 ml-2">
                  {src.count} <span className="text-slate-400 dark:text-slate-500 text-[10px]">({src.percentage.toFixed(0)}%)</span>
                </div>
              </button>
            );
          })}
