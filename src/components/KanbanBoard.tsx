        ...
        {FUNNEL_STAGES.map((stage) => {
          ...
          return (
            <div
              ...
              className="flex-1 bg-slate-100/70 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700/80 p-3 min-w-[220px] transition-colors duration-200"
            >
              {/* Column Header */}
              ...
              {/* Total Value */}
              <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mb-3 pb-2 border-b border-slate-200 dark:border-slate-700 flex justify-between">
                <span>Total:</span>
                <span className="font-bold text-slate-900 dark:text-slate-100">{formatCurrencyBRL(columnTotalValue)}</span>
              </div>
              ...
              <div className="space-y-2.5 min-h-[300px]">
                {columnLeads.map((lead) => (
                  <div
                    ...
                    className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xs hover:shadow-md transition-all cursor-grab active:cursor-grabbing group"
                  >
                    ...
                    <div className="text-xs font-extrabold text-blue-600 dark:text-blue-400 my-1">
                      {formatCurrencyBRL(lead.valorEstimado)}
                    </div>

                    <div className="text-[11px] text-slate-500 dark:text-slate-400 truncate" title={lead.servico || lead.queixaCliente}>
